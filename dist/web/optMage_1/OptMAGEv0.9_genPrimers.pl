 ...
 open(INFILE,  "INPUTtarg.txt") or die "Can't open file: $!";
 open(OUTFILE, ">>OUTprimers.txt") or die "Can't open output.txt: $!";
 ...
 foreach (<INFILE>) {
     if ($_ =~ /^(\w+)\t(\S+)\t(\d+)\t(\d+)\t(\d+)\t(\w+)\t(\w+)/) {
        $id = $1; $td = $2; $rep = $3; $lpos = $4; $rpos = $5; $mut = $6; $seq = $7;
        if ($td =~ m/\+/) {
        ...
        print OUTFILE "$id"."_vf\t$fpos_start\t$fpos_end\t$td\tforward\t$flength\t$f_primerseq_opt\t$f_primerseq_Tm\n";
        print OUTFILE "$id"."_vr\t$rpos_start\t$rpos_end\t$td\treverse\t$rlength\t$r_primerseq_opt\t$r_primerseq_Tm\n";
        }
 }
